...
function Contact({ Choose, inputRef }) {
    ...
    const filterContacts = (keyWord) => {
        ...
        return contactsData.filter((contact) => {
            const displayName = contact.otherMember[0]._id.name + " " + contact.otherMember[0]._id.lastName;
    ...
    const handleSerch = (value) => {
        ...
        if (value.length < 1) {
            ...
            fliter.sort((a, b) => a.otherMember[0]._id.name.localeCompare(b.otherMember[0]._id.name));
            setFiltered(fliter)
            setIsSearching(true)
        }
    }
                        ...
                        <div id="scrollable-content" className="scrollbar-telegram overflow-y-auto h-[calc(100vh-60px)] ">
                            ...
                            <List className="p-2 gap-1">
                                {contactsData.map((user) => {
                                    if (!user || user.contactType !== "person") return null;
                                    return (
                                        <ListItem
                                            key={user._id}
                                            onClick={() => {
