...
function AllGroups({ Choose, inputRef }) {
    const { contacts, backendUser, getLeftOwnedGroups, joinGroupAgain } = useContext(AuthContext);
    console.log("contacts", contacts)
    const getGroups = () => {
        const foundGroups = contacts.filter(contact => contact.contactType === "group").map((contact) => {
    ...
    const [leftOwnedGroups, setLeftOwnedGroups] = useState([]);
    const [joiningId, setJoiningId] = useState(null);

    useEffect(() => {
        const fetchLeftGroups = async () => {
            try {
                const res = await getLeftOwnedGroups();
                if (res && res.status === 200) {
                    setLeftOwnedGroups(res.data.data);
                }
            } catch (err) {
                console.error("Error fetching left groups:", err);
            }
        };
        fetchLeftGroups();
    }, [contacts]);
    ...
    useEffect(() => {
        const contactData = getGroups();
        const leftMapped = leftOwnedGroups.map(group => {
        ...
        const allGroups = [...contactData, ...leftMapped];

        const shortedContacts = allGroups.sort((a, b) => {
            return (a.name || "").localeCompare(b.name || "");
        });
        setGroupsData(shortedContacts);
    }, [contacts, leftOwnedGroups]);

    const handleJoinAgain = async (groupId) => {
                    ...
                    {groupsData.length > 0 && (
                        <div id="scrollable-content" className="scrollbar-telegram overflow-y-auto h-[calc(100vh-60px)] ">
                            ...
                            <List className="p-2 gap-1">
                                {groupsData.map((user) => {
                                    if (!user || user.contactType !== "group") return null;
                                    return (
                                        <ListItem
                                            key={user._id}
                                            onClick={() => {
                                            ...
                                            <div className="flex flex-col flex-1 overflow-hidden">
                                                ...
                                                {user.isLeftOwned ? (
                                                    ...
                                                    <span className="text-gray-700 text-[13px] font-medium truncate">
                                                        {user.members.length + ` member${user.members.length > 1 ? 's' : ""}`}
                                                    </span>
                                                )}
